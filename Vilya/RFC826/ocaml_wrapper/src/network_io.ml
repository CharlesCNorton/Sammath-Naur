...
let open_arp_socket ?(timeout=1) interface =
  try
    let fd = create_raw_socket interface in
    set_socket_timeout fd timeout;
    let mac_addr = get_interface_mac interface in
    let ip_addr = get_interface_ipv4 interface in
    { fd; interface; mac_addr; ip_addr }
  with Failure msg ->
    raise (Socket_error msg)
...
let send_arp_frame socket arp_payload target_mac =
  try
    let dst = if Bytes.length target_mac = 6 then target_mac
              else Bytes.make 6 '\xFF' in
    let frame = construct_ethernet_frame dst socket.mac_addr 0x0806 arp_payload in
    send_ethernet_frame socket.fd frame (Bytes.length frame)
  with Failure msg ->
    raise (Socket_error msg)
...
let receive_arp_frame socket =
  ...
  try
    let recv_len = recv_ethernet_frame socket.fd buffer in
    if recv_len = 0 then
      None
    else if recv_len < 42 then
      None
    else begin
      let frame = parse_ethernet_frame buffer recv_len in
      if frame.ethertype = 0x0806 then
        Some frame.payload
      else
        None
    end
  with
  | Failure msg when String.equal msg "Interrupted system call" -> None
  | Failure msg -> raise (Socket_error msg)

let sync_arp_entry_to_kernel socket ip_bytes mac_bytes =
  try
    netlink_update_arp socket.interface ip_bytes mac_bytes
  with Failure msg ->
    Printf.eprintf "Warning: Failed to sync to kernel ARP table: %s\n" msg;
    flush stderr
