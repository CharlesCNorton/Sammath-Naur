...
let validate_rarp_packet packet my_mac =
  if if if if is_valid_opcode packet.arp_op
     ...
     then if N.eqb packet.arp_op rARP_OP_REQUEST
          then if mac_eq packet.arp_tha my_mac
               then is_zero_ipv4 packet.arp_spa
               else false
          else true
     else false
  then if N.eqb packet.arp_op rARP_OP_REPLY
       then mac_eq packet.arp_tha my_mac
       else true
  else false
...
let process_rarp_packet ctx packet =
  if validate_rarp_packet packet ctx.arp_my_mac
  then if N.eqb packet.arp_op rARP_OP_REQUEST
       then if mac_eq packet.arp_tha ctx.arp_my_mac
...
let process_probe_timeout ctx probe current_time =
  ...
  then if N.ltb probe.probe_count aRP_PROBE_NUM
       then let new_probe = { probe_ip = probe.probe_ip; probe_count =
            ...
            let ctx' = { earp_my_mac = ctx.earp_my_mac; earp_my_ip =
              ctx.earp_my_ip; earp_cache = ctx.earp_cache; earp_state_data =
              (StateProbe new_probe); earp_iface = ctx.earp_iface;
            ...
            in
            let ctx' = { earp_my_mac = ctx.earp_my_mac; earp_my_ip =
              probe.probe_ip; earp_cache = ctx.earp_cache; earp_state_data =
              (StateAnnounce announce); earp_iface = ctx.earp_iface;
              earp_flood_table = ctx.earp_flood_table;
              earp_last_cache_cleanup = ctx.earp_last_cache_cleanup }
            in
            ctx',None
...
let process_announce_timeout ctx announce current_time =
  ...
  then if N.ltb announce.announce_count aRP_ANNOUNCE_NUM
       then let new_announce = { announce_count =
            ...
            let ctx' = { earp_my_mac = ctx.earp_my_mac; earp_my_ip =
              ctx.earp_my_ip; earp_cache = ctx.earp_cache; earp_state_data =
              (StateAnnounce new_announce); earp_iface = ctx.earp_iface;
              earp_flood_table = ctx.earp_flood_table;
              earp_last_cache_cleanup = ctx.earp_last_cache_cleanup }
            in
...
let process_arp_packet_enhanced ctx packet current_time elapsed_since_last =
  ...
  then ctx_aged,None
        ...
        | StateProbe probe ->
          ...
          then let ctx' = { earp_my_mac = ctx_aged.earp_my_mac; earp_my_ip =
                 ctx_aged.earp_my_ip; earp_cache = ctx_aged.earp_cache;
               ...
               in
               ctx',None
          else let i_am_target = ip_eq packet.arp_tpa ctx_aged.earp_my_ip in
               let cache' =
                 rfc826_merge ctx_aged.earp_cache packet.arp_spa
                   packet.arp_sha (Npos (XO (XO (XI (XI (XO (XI (XO (XO
                   XH))))))))) i_am_target
               in
        ...
        | _ ->
          ...
          then process_conflict ctx_aged current_time
               ...
               let ctx' = { earp_my_mac = ctx_aged.earp_my_mac; earp_my_ip =
                 ctx_aged.earp_my_ip; earp_cache = cache'; earp_state_data =
                 new_state; earp_iface = ctx_aged.earp_iface;
                 earp_flood_table = ctx_aged.earp_flood_table;
                 earp_last_cache_cleanup = ctx_aged.earp_last_cache_cleanup }
               in
               ...
               then if N.eqb packet.arp_op aRP_OP_REQUEST
                    then let reply =
                           make_arp_reply ctx_aged.earp_my_mac
                             ctx_aged.earp_my_ip packet.arp_sha packet.arp_spa
                         in
                         ctx',(Some reply)
                    else ctx',None
               else ctx',None)
...
| Timeout elapsed ->
  ...
  node.node_ctx.arp_my_ip; arp_cache = aged_cache }; node_time =
  (N.add node.node_time elapsed) }
